...
from utils.tasks import send_email_code
...
class postEmailCodeView(View):
    ...
    def post(self, request):
        ...
        try:
            email = Info["email"]
            code = Info["code"]
            if  EmailVerifyRecord.objects.get(email=email).code == code:
                return JsonResponse({'success': 'ok'})
            return JsonResponse({'success': 'no'})
        except:
            return JsonResponse({'success': 'no'})
...
class retPwdView(View):

    def post(self, request):
        email = json.loads(request.body.decode('utf-8'))["email"]
        password = json.loads(request.body.decode('utf-8'))["pwd"]
        if email:
